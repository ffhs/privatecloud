 ...
 * ActivityServer
 * 
 * Diese Activity wird verwendet um einen neuen Server zu erstellen oder einen bestehenden Server zu bearbeiten.
 *  
 * @author         Pascal Bieri
 */
public class ActivityServer extends Activity {
	...
	public void loadValues() {
		Log.d("jada", "test" + server.getServername() + server.getUsername()
				+ server.getPassword());
		...
		if (server.getPassword() != "" && server.getPassword() != null
				&& server.getPassword().toString() != "") {
	...
	public void onButtonClicked(View v) {
		...
		case R.id.server_Button_Genkey:
			...
			Toast.makeText(ActivityServer.this, "New keypair calculated",
		...
		case R.id.Server_Button_delete:
			...
			builder.setMessage("Are you sure to delete the server?")
	...
	private void savesettings() {
		if (isEmpty(port) || isEmpty(hostname) || isEmpty(servername)
				|| isEmpty(username) || isEmpty(remoteroot)
				|| (passauth.isChecked() && keyauth.isChecked())) {
			Toast.makeText(ActivityServer.this,
					"Not all required information supplied", Toast.LENGTH_LONG)
					.show();
		} else {
